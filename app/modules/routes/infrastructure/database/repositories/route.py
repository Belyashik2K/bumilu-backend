...
class RouteDataMapper:
    ...
    def sync_domain_to_model(entity: Route, model: RouteModel) -> None:
        ...
        if entity.translations is not None:
            RouteDataMapper._merge_translations(entity, model)

        if entity.points is not None:
            RouteDataMapper._merge_points(entity, model)
    ...
    def _merge_translations(entity: Route, model: RouteModel) -> None:
        ...
        domain_ids = {str(item.id.value) for item in domain_items}

        # удалить отсутствующие в домене
        model.translations[:] = [
            orm_item
            for orm_item in model.translations
            if str(orm_item.id) in domain_ids
        ]

        orm_by_id = {str(item.id): item for item in model.translations}

        for domain_item in domain_items:
            orm_item = orm_by_id.get(str(domain_item.id.value))

            if orm_item is None:
                orm_item = RouteTranslationModel(
                ...
                model.translations.append(orm_item)
                continue
    ...
    @staticmethod
    def _merge_points(entity: Route, model: RouteModel) -> None:
        domain_items = entity._points

        domain_ids = {str(item.id.value) for item in domain_items}

        model.points[:] = [
            orm_item for orm_item in model.points if str(orm_item.id) in domain_ids
        ]

        orm_by_id = {str(item.id): item for item in model.points}

        for domain_item in domain_items:
            orm_item = orm_by_id.get(str(domain_item.id.value))

            if orm_item is None:
                orm_item = RoutePointModel(
                    id=domain_item.id.value,
                    route_id=entity.id.value,
                    place_id=domain_item.place_id.value,
                    point_index=domain_item.index.value,
                )
                model.points.append(orm_item)
                continue

            orm_item.place_id = domain_item.place_id.value
            orm_item.point_index = domain_item.index.value
